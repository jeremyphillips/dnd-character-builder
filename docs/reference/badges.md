...
| File | Role |
|---|---|
| `packages/mechanics/src/combat/presentation/badges/action/combat-action-badges.types.ts` | `ActionBadgeKind`, `ActionBadgeDescriptor` |
| `packages/mechanics/src/combat/presentation/badges/action/combat-action-badges.ts` | `deriveCombatActionBadges()` -- pure derivation |
| `packages/mechanics/src/combat/presentation/badges/__tests__/action/combat-action-badges.test.ts` | Unit tests (21 cases) |
| `packages/mechanics/src/combat/presentation/actions/action-presentation.types.ts` | `ActionPresentationViewModel`, `ActionSemanticCategory`, `ActionSourceTag` |
| `packages/mechanics/src/combat/presentation/actions/action-presentation.ts` | `deriveActionPresentation()` -- wraps badge derivation with name, category, sourceTag |
| `packages/mechanics/src/combat/presentation/badges/__tests__/action/action-presentation.test.ts` | Unit tests (29 cases) |
...
| File | Role |
|---|---|
| `packages/mechanics/src/conditions/effect-condition-definitions.ts` | `EFFECT_CONDITION_DEFINITIONS` -- PHB condition data; `DAMAGE_IMPLIES_CONDITION` -- damage→condition dedup map |
| `packages/mechanics/src/combat/state/conditions/condition-rules/condition-definitions.ts` | `CONDITION_RULES` -- mechanical consequence rules |
| `packages/mechanics/src/combat/presentation/effects/presentable-effects.types.ts` | `CombatStateTone`, `CombatStatePriority`, `CombatStateSection`, `CombatStatePresentation`, `EnrichedPresentableEffect` |
| `packages/mechanics/src/combat/presentation/effects/presentable-effects.ts` | `collectPresentableEffects`, `enrichPresentableEffects`, `sortByPriority`, `groupBySection` |
| `packages/mechanics/src/combat/presentation/effects/combat-state-ui-map.ts` | `COMBAT_STATE_UI_MAP` -- merged lookup from condition definitions + engine markers |
| `packages/mechanics/src/combat/presentation/badges/defense/encounter-defense-badges.ts` | `buildEncounterDefensePreviewChips` -- defense preview chips with filtering options |
...
| `packages/mechanics/src/combat/presentation/formatters/turn-duration.ts` | `formatTurnDuration` -- source of truth; `src/features/combat/presentation/format-turn-duration.ts` re-exports for client combat UI |
...
(`packages/mechanics/src/combat/presentation/formatters/turn-duration.ts`; also re-exported from `src/features/combat/presentation/format-turn-duration.ts`). It accepts
...
Defined in `packages/mechanics/src/combat/presentation/effects/presentable-effects.types.ts`. Reused by `ActionBadgeDescriptor.tone`.
...
`packages/mechanics/src/conditions/effect-condition-definitions.ts` exports
...
Presentation resolves in order: **core** → **specialized** → **fallback** (`resolveEffectPresentation` in [`combat-state-ui-map.ts`](../../packages/mechanics/src/combat/presentation/effects/combat-state-ui-map.ts)).

| Tier | Source | Examples |
|------|--------|----------|
| **Core** | [`core-combat-state-presentation.ts`](../../packages/mechanics/src/combat/presentation/effects/core-combat-state-presentation.ts) | PHB `EffectConditionId` rows, immunity-only condition ids (e.g. `exhaustion`), universal engine markers (`bloodied`, `concentrating`, `banished`) |
| **Specialized** | [`specialized-effect-presentation.ts`](../../packages/mechanics/src/combat/presentation/effects/specialized-effect-presentation.ts) | Named monster/affliction/system-detail markers (`mummy-rot`, `engulfed`, `limb-severed`, …) — not universal PHB statuses |
...
**Defense badges** use a separate path (`presentationTier: 'defense'`): [`formatDamageDefenseLabel`](../../packages/mechanics/src/combat/presentation/badges/defense/encounter-defense-badges.ts) from semantic `level` + `damageType`, not raw `DamageResistanceMarker.label`.

- **Merged lookup** [`COMBAT_STATE_UI_MAP`](../../packages/mechanics/src/combat/presentation/effects/combat-state-ui-map.ts) = core ∪ specialized (backward-compatible `getCombatStatePresentation`).
...
- **Tests** ([`presentation-map-coverage.test.ts`](../../packages/mechanics/src/combat/presentation/effects/__tests__/presentation-map-coverage.test.ts)): core keys and specialized keys must not hit generic fallback; unknown keys may; **`FALLBACK_ONLY_PRESENTATION_KEYS`** allowlists keys that never get rows (e.g. dynamic ids).
