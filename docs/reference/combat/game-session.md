...
- **Tactical space at encounter start (parity):** Game session combat uses **`resolveEncounterSpaceForGameSessionStart`** (server). The Encounter Simulator uses **`resolveEncounterSpaceForSimulatorStart`** (client) plus shared **`pickEncounterGridMap`** / **`buildFallbackEncounterSpace`** so **map choice and fallback grid** match the same precedence. See [glossary.md § Encounter Simulator](./glossary.md#encounter-simulator).
