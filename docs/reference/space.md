...
- **Edges and boundaries** — `EncounterEdge`, `EncounterCell.kind` (e.g. `wall`), and `EncounterAuthoringPresentation.edgeEntries` (walls / doors / windows as presentation); not folded into `GridObject` unless a future feature explicitly bridges them. **Door** segments authored on a location map may persist **`doorState`** (open/closed and lock metadata on `LocationMapEdgeAuthoringEntry`); when an encounter space is built from that map, **movement and line-of-sight blocking** on the corresponding `EncounterEdge` follow **closed vs open** (see §3 Grid creation). **Lock** fields are authoring/UI data in the current milestone — they do not yet drive encounter permissions or prompts.
...
**Authored map edges (interior segments):** `buildEncounterSpaceFromLocationMap` also walks **`LocationMap.edgeEntries`** and, for each interior **`between:`** segment that exists in the combat grid, appends an **`EncounterEdge`**. **Walls** and **windows** use coarse **`kind`**-only rules (unchanged). **Doors** that resolve as door instances use the door family’s registry runtime as the **closed** baseline and **`resolveDoorRuntimeFromState`** (`src/features/.../locationPlacedObject.runtime.ts`) together with optional authored **`doorState`**: a **closed** door (including omitted state, which defaults to closed) **blocks** movement and sight on that segment; an **open** door clears **`blocksMovement`** and **`blocksLineOfSight`** on the edge. **Perimeter** edge ids are skipped for encounter adjacency (same as before). This is distinct from **`GridObject`** hydration; door *variants* remain material/form only — open/lock are instance fields on the edge row, not extra palette variants.
...
- **Edges:** Consecutive cells on the path are checked with **`segmentSightBlocked`** (`spatial/edgeCrossing.ts`). Orthogonal steps use the edge between the two cells (if absent, the boundary is open). **Diagonal** steps use a **strict corner rule for rays**: sight is blocked if **either** supporting orthogonal segment would block sight. **Movement** diagonals do **not** use this rule — they use **orthogonal decomposition** in `movementReachability.ts` (see §3 Movement). **Window** example: an edge may set **`blocksMovement: true`** and **`blocksSight: false`** (blocks walking through the sill, not the sight line). **Door** edges from a location map carry **`blocksSight`** / **`blocksMovement`** consistent with **closed vs open** authoring (see §3 Grid creation); an open door clears both on the segment.
...
**Transitional source-of-truth:** resolution still reads **`EncounterCell`** flags (e.g. `blocksMovement`, `blocksSight`, `kind`), **`GridObject`** on **`EncounterSpace.gridObjects`**, and **`EncounterEdge`**. Door edge blocking/LoS on **`EncounterEdge`** is derived from authored **`doorState`** when the space is built from a map (see §3 Grid creation). Denormalized cell flags are **compatibility** inputs from some legacy/hydration paths — they are not documented as the sole long-term authority when edges or grid objects also describe the same feature.
...
| `AuthoredDoorState` / `sanitizeAuthoredDoorState` | `shared/domain/locations/map/locationMapDoorAuthoring.*` | Optional open/lock fields on **`LocationMapEdgeAuthoringEntry`**; canonical defaults and invalid-combo sanitization. |
| `resolveDoorRuntimeFromState` | `src/features/content/locations/domain/model/placedObjects/locationPlacedObject.runtime.ts` | Effective door **runtime** slice for encounter edges: closed uses registry baseline; open clears movement + LoS blocking (lock does not affect blocking in the current milestone). |
