...
- **Client:** The **Encounter Simulator** uses **`startEncounterFromSetup`** for encounter start and **`applyCombatIntent`** for migrated in-encounter actions (see [client/local-dispatch.md](./client/local-dispatch.md)); active combat uses the shared **`CombatPlayView`** shell via **`useEncounterActivePlaySurface`**. **GameSession** (`src/features/game-session`) provides lobby, setup, lifecycle, and a **`/play`** route that loads persisted combat by **`activeEncounterId`**, hydrates **`useEncounterState`**, mirrors intents to the server, and reuses the same **`CombatPlayView`** surface—see [game-session.md](./game-session.md). For **`/play`**, **viewer seat** (DM vs player vs observer) and **controlled combatant ids** drive **turn capabilities** (`deriveEncounterCapabilities`); when **`GameSession.participants`** omits non-DM players (common), seat is **inferred** from **campaign roster ownership** and **party PC** combatants in the encounter state—see [client/encounter-viewer-permissions.md](./client/encounter-viewer-permissions.md).
- **Shared package:** [`@rpg-world-builder/mechanics`](../../../packages/mechanics/README.md) exposes the combat application seams and wire types (see [adr-shared-combat-extraction.md](./adr-shared-combat-extraction.md)), including **`resolveSessionControlledCombatantIds`** and helpers used for session seat policy.
- **Server:** REST endpoints persist **combat sessions** (MongoDB) with **`sessionId`**, **`revision`**, and **`EncounterState` snapshot**; startup and apply-intent are **separate** routes. When a **game session** references the combat via **`activeEncounterId`**, **`POST .../intents`** runs **authorization** aligned with the client (same seat resolution and controlled-combatant set, using **approved party roster** from **`getPartyCharacters`** for inference parity); forbidden intents return **403**. Realtime broadcast and **broad** campaign tenancy on combat routes are **not** part of the current minimal surface—see [server/authoritative-flow.md](./server/authoritative-flow.md) and [roadmap.md](./roadmap.md).
