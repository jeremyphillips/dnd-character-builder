...
The long-term direction is a **registry-driven** object authoring system: **toolbar-first place palette** (with category/group presentation in data), explicit **loaded** placement state (`activePlace` with optional variant id), **variants** (later phases), **edge placement** for doors/windows (and similar), and **inspection/configuration in the rail** after placement — without collapsing palette grouping into the persisted map model. **Phase 1 (palette foundation)** is implemented: `getPlacePaletteItemsForScale` feeds **`LocationMapEditorPlaceTray`** next to the toolbar; cell clicks use **`resolvePlacementCellClick`** in `placementRegistryResolver.ts` (consolidating `resolvePlacedKindToAction` + payload mapping). The Map rail shows a short hint in place mode instead of duplicating the palette. Later phases: **Phase 2 (variants):** [.cursor/plans/location-workspace/location_workspace_object_authoring_phase2_variants.plan.md](../../.cursor/plans/location-workspace/location_workspace_object_authoring_phase2_variants.plan.md). **Phase 3 (edge placement, placeholder):** [.cursor/plans/location-workspace/location_workspace_object_authoring_phase3_edge_placement.plan.md](../../.cursor/plans/location-workspace/location_workspace_object_authoring_phase3_edge_placement.plan.md). **Phase 4 (config and editing, placeholder):** [.cursor/plans/location-workspace/location_workspace_object_authoring_phase4_config_editing.plan.md](../../.cursor/plans/location-workspace/location_workspace_object_authoring_phase4_config_editing.plan.md).
...
| `place` | **Discrete** placement only: **linked content** (child locations) and **map objects** (props), per `LOCATION_SCALE_MAP_CONTENT_POLICY`. Palette: `getPlacePaletteItemsForScale` → toolbar **`LocationMapEditorPlaceTray`**; selection **`activePlace`** (`linked-content` \| `map-object`, optional `variantId`). Cell click: **`resolvePlacementCellClick`** (`placementRegistryResolver.ts`). Map rail: hint only (no duplicate palette). |
...
| `LocationMapEditorPlaceTray` | Shown when `mode === 'place'`; placeables from `getPlacePaletteItemsForScale` (sets `activePlace` only). |
