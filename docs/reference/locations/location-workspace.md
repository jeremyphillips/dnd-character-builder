...
**Authored base-map layer order (square editor and combat underlay should match intent):** cell fills / region tint → **paths** → **edges** → **authored cell objects** (cell-anchored **raster** imagery for registry props/markers, not MUI glyph icons). The path/edge SVG overlay is stacked **below** the cell grid (`SquareMapAuthoringSvgOverlay` z-order under `GridEditor`) so placed-object imagery inside cells remains visible. **Edge** doors/windows are drawn as **vector** segments on boundaries (same layer family as other edges), not as cell raster sprites. Object display rules align with `deriveLocationMapAuthoredObjectRenderItems` / `LocationMapAuthoredObjectRenderItem` in `shared/domain` (see `locationMapAuthoredObjectRender.helpers.ts`); resolution to URLs and layout is `resolvePlacedObjectCellVisual` → `PlacedObjectCellVisualDisplay` in `domain/presentation/map/`.
...
| Collection | Role |
|------------|------|
| **`cellEntries`** | Per-cell authoring: optional `linkedLocationId`, optional `objects[]`, optional **`cellFill`** — `{ familyId, variantId }` for terrain/surface paint (see **Cell fill registry** below). |
| **`pathEntries`** | Map-level **roads** and **rivers** as ordered authored chains: each entry has `id`, `kind` (`road` \| `river`), ordered `cellIds` (adjacent along the grid), and optional **`name`** / **`description`** for the Selection rail. |
...
| **Authoring cell chrome (`state → sx`)** | `components/mapGrid/mapGridAuthoringCellVisual.builder.ts` | Pure builders for square + hex visual layers; composed with `GridCellHost` / `GridCellVisual`. |
| **Cell host + visual primitives** | `components/mapGrid/GridCellHost.tsx`, `GridCellVisual.tsx` | Shared structure: native `<button type="button">` host when interactive (semantics, focus, keyboard activation, disabled); inner visual owns border/fill/hover and keyboard focus ring (not global primary `button` chrome on the host). |
...
**Rendering note:** Grid cells use **`GridCellHost`** (native `<button type="button">` when the cell is interactive) plus **`GridCellVisual`** for chrome from `mapGridAuthoringCellVisual.builder.ts` and `gridCellStyles` / `mapGridCellVisualState`. When Select-mode hover is suppressed for a cell (region/path/object/edge winner), the builder mirrors idle border/background on `:hover` so hover does not compete with the resolved target. Policy summary: `domain/authoring/editor/selectMode/selectModeChrome.policy.ts` (`SELECT_MODE_CHROME_POLICY_DOC`). **Combat** uses the same host/visual primitives with tactical `sx` from `features/combat/components/grid/cellVisualStyles.ts` (not merged with authoring chrome).
...
**Subfolders (ownership):** `header/`, `canvas/`, `leftTools/` (with `paint/` and `draw/` barrels), `rightRail/` with **`types/`** (type-only: `LocationEditorRailSection`, `LocationMapSelection`), **`locationEditorRail.helpers.ts`** (pure helpers: `selectedCellIdForMapSelection`, `mapSelectionEqual` — shared with grid and domain select-mode), **`tabs/location/`** (homebrew **`LocationTab`** and system **`SystemLocationTab`** — presentational grouping; form boundaries unchanged from route wiring), **`tabs/selection/`** (**`SelectionTab`** dispatcher, **`inspectors/`** for map entity UIs, **`templates/SelectionRailTemplate`** shared shell, **`fields/`** — specialty rail UI fragments (e.g. name/description, stairs), **`selectionRail.helpers.ts`**, **`viewModels/`** — shared `SelectionRailViewModel` types and `buildCellFillSelectionRailViewModel`), **`adapters/`** (draft normalization — e.g. region metadata), plus rail shell and tabs at `rightRail/` root. Map tools live on the canvas toolbar, not in the rail. **`routes/locationEdit/postAuthoringSelection.helpers.ts`** builds `mapSelection` + `selectedCellId` after successful place/path/edge/link authoring; **`useLocationEditWorkspaceModel`** focuses the **Selection** tab for those commits (changing the map toolbar mode does **not** switch the rail). **`onPlaceObjectStrokeEnd`** on **`LocationGridAuthoringSection`** focuses Selection once when drag-place ends. `rightRail/__tests__/` holds rail-local tests (e.g. `locationEditorRail.helpers.test.ts`). `setup/` holds the location create setup dialog. Top-level workspace files include the edit shells, `LocationEditorWorkspace`, **`LocationGridAuthoringSection`** (workspace-level map authoring orchestrator), and `BuildingFloorStrip`. **Authoring helpers** live under **`components/authoring/draft/`** (grid draft types + persist/compare utilities) and **`components/authoring/geometry/`** (overlay geometry and path-rendering adapters). **`components/index.ts`** re-exports map toolbar and rail panels from **`workspace/leftTools`** and **`workspace/rightRail/...`**.

The workspace is composed of feature-owned components:

| Component | Role |
|-----------|------|
| `LocationEditHomebrewWorkspace` | **Homebrew (user-authored) edit** — persisted `source === 'campaign'` (including building + floor): wraps `FormProvider`, optional `BuildingFloorStrip` + map column, `LocationEditorWorkspace`, location form + visibility in the rail, delete `ConfirmModal`. |
...
| `LocationEditorRailSectionTabs` | Right-rail tabs: **Location** (location metadata forms) and **Selection** (map inspector). Map authoring tools are on the **left toolbar** next to the canvas, not in this rail. Section state is separate from toolbar mode. |
| `SelectionTab` | Selection tab composition root: switches on `mapSelection` to cell / object / path / edge / edge-run / region inspectors. |
...
The two right-rail tabs (**Location** and **Selection**) are not separate dirty stores: they feed the shared form, `LocationGridDraftState`, and (for buildings) stair-connection state. **Non-persisted** UI such as toolbar mode, paint selection, and `mapSelection` is not part of the persistable snapshot (see `authoring/draft/locationGridDraft.utils.ts`).
...
1. **Region metadata (Selection → region):** [`LocationMapRegionMetadataForm`](src/features/content/locations/components/workspace/rightRail/tabs/selection/inspectors/LocationMapRegionMetadataForm.tsx) leads with [`SelectionRailIdentityBlock`](src/features/content/locations/components/workspace/rightRail/tabs/selection/templates/SelectionRailTemplate.tsx) (**Map** / region **name** or “Region” / **Overlay region**), then uses RHF for field UX; **`onPatchRegion(regionId, patch)`** writes through [`regionMetadataDraftAdapter.ts`](src/features/content/locations/components/workspace/rightRail/adapters/regionMetadataDraftAdapter.ts) into **`gridDraft.regionEntries`**. **Description** uses [`useDebouncedPersistableField`](src/ui/hooks/useDebouncedPersistableField.ts) for typing ergonomics, with **flush** on region change (`useLayoutEffect`), rail tab unmount, and via **`flushDebouncedPersistableFieldsRef`** before header Save (wrapped in **`flushSync`** in [`LocationEditRoute.tsx`](src/features/content/locations/routes/LocationEditRoute.tsx) so `gridDraftRef` matches before submit). No separate “apply” step for persistable data.
2. **Stair pairing / stair endpoint:** [`stairEndpointFields.tsx`](src/features/content/locations/components/workspace/rightRail/tabs/selection/fields/stairEndpointFields.tsx) (composed by [`LocationMapObjectInspector`](src/features/content/locations/components/workspace/rightRail/tabs/selection/inspectors/LocationMapObjectInspector.tsx)) uses **`FormProvider` + `useForm`** for controlled fields and **`onAfterChange`** / **Link endpoints** to update **`gridDraft`** (e.g. `onUpdateCellObjects`). RHF handles inputs; **persistable** updates are immediate draft writes, not a nested HTML form submit to the workspace.
3. **Placed objects (Selection → object / edge):** [`SelectionRailTemplate`](src/features/content/locations/components/workspace/rightRail/tabs/selection/templates/SelectionRailTemplate.tsx) is the shared rail shell: **`categoryLabel`**, **`title`**, optional **`placementLine`**, optional **`metadata`** (kind-specific blocks, presentation rows via [`SelectionMetadataRows`](src/features/content/locations/components/workspace/rightRail/tabs/selection/templates/SelectionRailTemplate.tsx)), optional **`children`** (linked location title, freeform **Label** field, edge label field — specialty UI rather than a large prop surface), optional **`actionsSlot`**, **`onRemoveFromMap`**. Registry rows use [`getDefaultVariantPresentationForKind`](src/features/content/locations/domain/model/placedObjects/locationPlacedObject.selectors.ts) and [`presentationRowsFromPresentation`](src/features/content/locations/components/workspace/rightRail/tabs/selection/selectionRail.helpers.ts); cell objects may resolve kind via [`resolvePlacedObjectKindForCellObject`](src/features/content/locations/domain/model/placedObjects/locationPlacedObject.selectors.ts) where needed. Pure helpers: [`selectionRail.helpers.ts`](src/features/content/locations/components/workspace/rightRail/tabs/selection/selectionRail.helpers.ts). **Empty-cell** selection uses only [`SelectionRailIdentityBlock`](src/features/content/locations/components/workspace/rightRail/tabs/selection/templates/SelectionRailTemplate.tsx) plus host/ancestry context in [`CellSelectionInspector`](src/features/content/locations/components/workspace/rightRail/tabs/selection/inspectors/CellSelectionInspector.tsx) — **no** linked-location picker or add/remove cell objects there (those belong on object selection and other tools). **Edge** door/window inspectors: today the map wire stores **`edgeId` + `kind`** only for some cases; extended edge-authored fields are planned (see [.cursor/plans/location-workspace/location_workspace_object_authoring_phase4_config_editing.plan.md](../../.cursor/plans/location-workspace/location_workspace_object_authoring_phase4_config_editing.plan.md)).
4. **Paths (Selection → path):** [`LocationMapPathInspector`](src/features/content/locations/components/workspace/rightRail/tabs/selection/inspectors/LocationMapPathInspector.tsx) uses the same **`SelectionRailTemplate`** backbone (no raw path id in the primary block).
...
The long-term direction is a **registry-driven** object authoring system: **toolbar-first place palette** (with category/group presentation in data), explicit **loaded** placement state (`activePlace`), **family-scoped variant ids** (object-authoring Phase 2), **edge placement** for doors/windows (and similar), and **inspection/configuration in the rail** after placement — without collapsing palette grouping into the persisted map model. Registry **`category`** on each family is **UI palette grouping only** (buckets such as `structure`, `furniture`, `treasure` in `locationPlacedObject.registry.ts`); resolver routing uses **`linked-content` \| `map-object`** on **`activePlace`**, not the palette bucket. Variant **presentation** hints (`presentation.material`, …) align with shared **`MaterialId`** / **`MATERIAL_META`** in `shared/domain/materials/materials.ts` (canonical ids + optional **reaction** / **structural** profiles — not persisted on the map wire today). **Object-authoring Phase 1 (palette foundation)** and **Phase 2 (variants)** are implemented: each registry family has **`defaultVariantId`** plus a **`variants`** map; **loaded** state is **`kind` (family id) + `variantId`** (both required). **Persisted** cell objects use **`LocationMapObjectKindId`**, optional **`authoredPlaceKindId`**, and optional **`variantId`** (omit on legacy maps → **`resolvePlacedObjectVariant`** uses the family default). **`resolvePlacementCellClick`** seeds **`variantId`** for new objects; **`cellDraftToCellEntries`** writes it under **`cellEntries[].objects[]`**. **Map-object** families with multiple variants show a **count badge** and a **secondary menu** (popover) to pick a variant; **primary click** always loads the **default** variant for fast repeat placement. **Linked-content** rows (`city`, `site`) stay **single-variant** in the palette (no multi-variant picker there). The **building** family is **map-object** (footprint variants, placed as `marker` + `authoredPlaceKindId: 'building'` — not a linked-building modal). `getPlacePaletteItemsForScale` feeds **`LocationMapEditorPlaceTray`**; cell clicks use **`resolvePlacementCellClick`** in `placementRegistryResolver.ts` (consolidating `resolvePlacedKindToAction` + payload mapping). Later phases: **Phase 3 (edge placement)** — implemented; normative plan: [.cursor/plans/location-workspace/location_workspace_object_authoring_phase3_edge_placement.plan.md](../../.cursor/plans/location-workspace/location_workspace_object_authoring_phase3_edge_placement.plan.md). **Phase 4 (config and editing in the rail)** — **shipped** for registry-backed presentation metadata, cell/edge inspectors, and simplified empty-cell panel; **edge wire** remains **`{ edgeId, kind }`** until edge-authored instance parity lands; normative plan and follow-on work: [.cursor/plans/location-workspace/location_workspace_object_authoring_phase4_config_editing.plan.md](../../.cursor/plans/location-workspace/location_workspace_object_authoring_phase4_config_editing.plan.md).

**Placed object imagery (raster, not MUI glyphs):** Registry variants use **`assetId`** (stable keys into a generated manifest), not `iconName`. **Phase 1** pipeline emits [`assets/system/locations/objects/location-objects.manifest.json`](../../assets/system/locations/objects/location-objects.manifest.json) and per-PNG trim metadata; **Phase 2** wires the app: **`locationPlacedObjectRasterAssets.ts`** resolves bundled PNG URLs; **`MapPlacePaletteItem`** carries **`previewImageUrl`** for **`LocationMapEditorPlaceTray`** (family tiles and variant popover use **`<img>`**); **in-map** cell objects use **`resolvePlacedObjectCellVisual`** / **`PlacedObjectCellVisualDisplay`** (**`<img>`** when the manifest has a **map** slice). **`placementMode: 'edge'`** families (doors/windows) keep **vector** boundary rendering on the map; the place palette still shows **preview images** for those variants. See [`assets/system/locations/objects/README.md`](../../assets/system/locations/objects/README.md). Broader migration plan: [.cursor/plans/sprite_placed_objects_migration_a4f3eeec.plan.md](../../.cursor/plans/sprite_placed_objects_migration_a4f3eeec.plan.md).

**Placed object footprint / layout (Phase 3, feet + square grid):** Variants may define **`footprint`** in **feet** on the registry (`placedObjectFootprint.types.ts`). **`resolveAuthoringCellUnitFeetPerCell`** maps **`LocationMap.grid.cellUnit`** → feet per cell for editor layout; **`resolvePlacedObjectFootprintLayoutPx`** combines that with **`cellPx`** to size in-map rasters (square authoring grids and combat underlay; **hex** does not use this pipeline yet). This is **presentation layout** only — not tactical occupancy or final hit-testing.

**Placed object sprite fit (Phase 4, table pilot):** In-map images use **`object-fit: contain`** inside the footprint box (`placedObjectMapSprite.constants.ts`) so sprites are not non-uniformly stretched; new real-world sizes get **new variants** with their own **`assetId`** + footprint (see [`assets/system/locations/objects/README.md`](../../assets/system/locations/objects/README.md) Phase 4). Registry ↔ manifest checks run in **`locationObjectsRegistryManifest.crossReference.test.ts`**; full asset gate: **`npm run validate:location-objects`** (see README Phase 7).

**Variant resolution (read / hydration):** `shared/domain/registry/familyVariantResolve.ts` defines `resolveFamilyVariant` for any registry shaped as `{ defaultVariantId, variants }` (invariant: `defaultVariantId` must be a key of `variants`). Placed-object wrappers **`resolvePlacedObjectVariant`** and **`normalizeVariantIdForFamily`** live in `locationPlacedObject.selectors.ts`. Prefer **`resolvePlacedObjectVariant`** when the variant id may be missing, invalid, or from persisted wire data; **`getPlacedObjectVariantPresentation`** / **`getPlacedObjectVariantLabel`** expect a **registry-valid** variant id for that family (no default fallback on those getters). Edge door/window authored identity is hydrated through **`resolveAuthoredEdgeInstance`** (`locationMapEdgeAuthoring.resolve.ts`). **Cell fills** use the same pattern: **`resolveCellFillVariant`** (`authoredCellFillDefinitions.ts`) on `{ familyId, variantId }` from persistence or the grid draft. Shared **tray** layout primitives (**`LocationMapEditorTrayScrollColumn`**, **`LocationMapEditorTraySectionHeading`**) for map editor palettes live under `components/workspace/leftTools/tray/` (place and paint shells compose them; domain palette data stays in helpers).
...
| **Control** | MUI vertical `ToggleButtonGroup` (exclusive), **icon-only** mode buttons (Select / Paint / Place / Draw / Erase — distinct from **place palette** imagery below) |
...
| `paint` | **Fill-like** authoring: paint **cell fills** into draft **`cellFillByCellId`** as **`{ familyId, variantId }`** per cell. Palette: **`getPaintPaletteItemsForScale`** (registry ∩ scale policy) → **`LocationMapEditorPaintTray`**; selection **`activePaint`** holds the chosen family/variant. Region painting is intended to extend this tool later. |
| `place` | **Discrete** placement only: **linked content** (child locations) and **map objects** (props), per `LOCATION_SCALE_MAP_CONTENT_POLICY`. Palette: `getPlacePaletteItemsForScale` → toolbar **`LocationMapEditorPlaceTray`**; selection **`activePlace`** (`linked-content` \| `map-object`, **`kind`** = family id, required **`variantId`**). Cell click: **`resolvePlacementCellClick`** (`placementRegistryResolver.ts`). |
| `draw` | **Line/boundary** authoring: **paths** and **edges** (per `LOCATION_SCALE_MAP_CONTENT_POLICY`). Palette: `getGroupedDrawPaletteForScale` → toolbar **`LocationMapEditorDrawTray`**; selection **`activeDraw`** (`path` \| `edge`). **Edge** rows use **`LOCATION_EDGE_FEATURE_KIND_META`** for **label/description only** (coarse `kind` copy — not variants or instance state). Path segments use `pathAnchorCellId`; edges use boundary-paint in `LocationGridAuthoringSection`. Resolver: `resolveDrawSelectionToAction`. |
...
**State hook:** `useLocationMapEditorState` holds `mode`, `activePaint`, `activePlace`, `activeDraw`, `pathAnchorCellId` (for **Draw → path**). Leaving **place** clears place selection; leaving **draw** clears draw selection and path anchor. **Linked-content** place families (`city`, `site`, registry `linkedScale`) place a **marker** object immediately; the campaign link target is chosen in **Selection → object inspector** via `OptionPickerField` (see `buildLinkedLocationPickerOptions` in `domain/authoring/editor/linkedLocationPickerOptions.ts`).
...
| `LocationMapEditorPlaceTray` | Shown when `mode === 'place'`; one row per **family** from `getPlacePaletteItemsForScale` (sets `activePlace`); tiles use **`previewImageUrl`** (bundled PNGs, not MUI icons); **variant count** + **popover** for multi-variant **map-object** families; **tooltips** from registry labels/descriptions. |
| `LocationGridAuthoringSection` | Renders `GridEditor` or `HexGridEditor`, applies fills, **placed-object raster overlays** (and linked-location affordances), map-editor pointer behavior, and SVG overlays for paths (both geometries) and edges (square only). |
...
1. Select a path tool (road or river) from the **Draw** tool (left toolbar).
...
**Palette filtering:** `useLocationEditWorkspaceModel` builds **place** items from `getPlacePaletteItemsForScale` (objects/links only). **Draw** items come from `getGroupedDrawPaletteForScale`; on **hex** geometry, only **path** kinds are shown (edges are square-first; see **Open issues**).
...
**Coarse `kind` vs door/window authored detail:** `kind` is one of `LOCATION_MAP_EDGE_KIND_IDS` (`wall` \| `window` \| `door`). **`LOCATION_EDGE_FEATURE_KIND_META`** (`domain/model/map/locationEdgeFeature.types.ts`) supplies **coarse** label/description (and optional **`edgeCategory`**, wall **`supportedMaterials`**) for tools that list edge kinds without the placed-object registry — today the **draw** edge palette reads **label + description** via `getDrawEdgePaletteItemsForScale`. It does **not** own window variants, door widths, lock states, or other instance fields; those belong to **`AUTHORED_PLACED_OBJECT_DEFINITIONS`**, **`resolveAuthoredEdgeInstance`**, and (when shipped) typed **`state`** on the edge row. **Policy:** at **floor** scale, **Draw → edge** exposes **`wall`** only (`LOCATION_SCALE_MAP_CONTENT_POLICY`); doors/windows on boundaries are placed via **Place** (`placementMode: 'edge'`) onto the same `edgeEntries` wire.
...
## Maintenance — orchestration cleanup (optional follow-ups)

**Plan (archive):** [.cursor/plans/location_workspace_cleanup_94269d45.plan.md](../../.cursor/plans/location_workspace_cleanup_94269d45.plan.md) — concern-based extractions for `LocationGridAuthoringSection`, `useLocationEditWorkspaceModel`, and route rail wiring; **does not** change dirty/save contracts. **Core work is done**; the items below are **optional** incremental polish, not prerequisites for “finishing” the editor.

### What landed (where to look)

| Concern | Location |
|--------|----------|
| Grid authoring orchestration (modes, overlays, hooks) | `components/workspace/LocationGridAuthoringSection.tsx` plus colocated modules (`useLocationGridAuthoringCellPointers`, `useLocationGridPaintStroke`, `useLocationGridSelectMode`, `locationGridAuthoringMapOverlayLayers`, …) |
| Session hook + map cell click / erase / path chains | `routes/locationEdit/useLocationEditWorkspaceModel.ts` |
| Map-selection delete helpers | `routes/locationEdit/mapSessionDraft.helpers.ts` |
| Building stair link/unlink + remove object (stair cleanup) | `routes/locationEdit/useLocationEditBuildingStairHandlers.ts` |
| Linked-location picker options (object inspector) | `domain/authoring/editor/linkedLocationPickerOptions.ts` |
...
These are **deliberately non-blocking**. Prefer **readability and ownership** over abstraction or line-count churn.
...
| Topic | Notes |
|-------|--------|
| **Hook size** | `useLocationEditWorkspaceModel` may remain large; treat further splits as **optional** (criteria above). Snapshot cost and `watch()` breadth are already noted under **Performance (homebrew snapshot)** and **Performance — workspace snapshot**. |
...
16. **Placed object raster assets:** generated manifest and PNGs under [`assets/system/locations/objects/`](../../assets/system/locations/objects/) (`location-objects.manifest.json`, `validate:location-objects-manifest` / `validate:location-objects` / `build:location-objects-manifest` in `package.json`); runtime URL resolution in `domain/model/placedObjects/locationPlacedObjectRasterAssets.ts`; broader migration narrative: [.cursor/plans/sprite_placed_objects_migration_a4f3eeec.plan.md](../../.cursor/plans/sprite_placed_objects_migration_a4f3eeec.plan.md).
