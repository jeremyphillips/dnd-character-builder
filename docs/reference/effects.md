...
- **Catalog audit:** run `npm run test:run -- src/features/encounter/helpers/spells/spell-catalog-audit.test.ts` for merged-system-spell metrics (stranded counts, ambiguous delivery, explicit `save.dc`, etc.). Counts are for reporting, not CI gates; see `spells/spell-resolution-audit.ts` and [resolution.md](./resolution.md) §7 “Spell combat adapter”.
