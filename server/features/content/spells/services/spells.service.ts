...
function validateInput(body: Record<string, unknown>): ValidationError[] {
  ...
  if (typeof body.school !== 'string' || !VALID_SCHOOLS.includes(body.school)) {
    errors.push({ path: 'school', code: 'INVALID_VALUE', message: `school must be one of: ${VALID_SCHOOLS.join(', ')}` });
  }
...
export async function create(
...
): Promise<{ spell: CampaignSpellDoc } | { errors: ValidationError[] }> {
  ...
  if (existing) {
    return {
      errors: [{ path: 'spellId', code: 'DUPLICATE', message: `A spell with id "${spellId}" already exists in this campaign` }],
    };
  }
...
export async function update(
  ...
  spellId: string,
  body: Record<string, unknown>,
): Promise<{ spell: CampaignSpellDoc } | { errors: ValidationError[] } | null> {
  ...
  const $set: Record<string, unknown> = { name, description, imageKey, school, level };
