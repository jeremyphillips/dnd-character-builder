...
| [location_workspace_object_authoring_phase3_edge_placement.plan.md](location_workspace_object_authoring_phase3_edge_placement.plan.md) | **Child (Phase 3)** | **Edge placement** — `placementMode` (`cell` / `edge`) in shared registry; armed state familyKey + variantId + target; resolver-owned cell/edge paths; square-first; `edgeEntries` wire unchanged; minimal selected-edge rail. Hex edge authoring not assumed. **Not** Phase 4 deep config. |
| [location_workspace_object_authoring_phase4_config_editing.plan.md](location_workspace_object_authoring_phase4_config_editing.plan.md) | **Child (Phase 4)** | **Config and editing** — shared placed-object rail template, inspector ownership, Label rules, persistence notes, **edge-authored instance modeling** (long-term `edgeEntries` parity vs cell objects), `location-workspace.md` updates. **Not** full workspace/runtime redesign. |
| [location_workspace_object_authoring_phase4_build_plan.md](location_workspace_object_authoring_phase4_build_plan.md) | **Phase 4 execution** | **Implementation build plan** — milestones M1–M8 (**M8** = post-build cleanup: object-first edge inspectors, `presentation` metadata, empty-cell rail). Normative spec remains Phase 4 config/editing plan. |
