...
export type CampaignSkillProficiencyInput = {
  skillProficiencyId?: string;
  name: string;
  description?: string;
  ability: string;
...
function validateInput(body: Record<string, unknown>): ValidationError[] {
  ...
  if (typeof body.ability !== 'string' || !VALID_ABILITIES.includes(body.ability as (typeof VALID_ABILITIES)[number])) {
    errors.push({
      path: 'ability',
      code: 'INVALID_VALUE',
      message: `ability must be one of: ${VALID_ABILITIES.join(', ')}`,
    });
  }
  ...
  if (body.suggestedClasses !== undefined) {
    if (!Array.isArray(body.suggestedClasses)) {
      errors.push({
        path: 'suggestedClasses',
        code: 'INVALID_TYPE',
        message: 'suggestedClasses must be an array',
      });
    } else if (body.suggestedClasses.some((x) => typeof x !== 'string')) {
      errors.push({
        path: 'suggestedClasses',
        code: 'INVALID_TYPE',
        message: 'suggestedClasses must be an array of strings',
      });
    }
  }
...
function toDoc(doc: Record<string, unknown>): CampaignSkillProficiencyDoc {
  return {
    ...
    suggestedClasses: Array.isArray(doc.suggestedClasses)
      ? (doc.suggestedClasses as string[])
      : [],
...
export async function listByCampaign(
  campaignId: string,
): Promise<CampaignSkillProficiencyDoc[]> {
  const docs = await CampaignSkillProficiency.find({ campaignId })
    .sort({ name: 1 })
    .lean();
...
export async function getById(
  campaignId: string,
  skillProficiencyId: string,
): Promise<CampaignSkillProficiencyDoc | null> {
  const doc = await CampaignSkillProficiency.findOne({
    campaignId,
    skillProficiencyId,
  }).lean();
...
export async function create(
  ...
  { skillProficiency: CampaignSkillProficiencyDoc } | { errors: ValidationError[] }
> {
  const errors = validateInput(body);
  if (errors.length > 0) return { errors };

  const name = (body.name as string).trim();
  const skillProficiencyId =
    (body.skillProficiencyId as string | undefined)?.trim() ||
    generateSkillProficiencyId(name);
  const description = ((body.description as string) ?? '').trim();
  const ability = body.ability as string;
  const suggestedClasses = Array.isArray(body.suggestedClasses)
    ? (body.suggestedClasses as string[])
    : [];
  ...
  const existing = await CampaignSkillProficiency.findOne({
    campaignId,
    skillProficiencyId,
  }).lean();
  if (existing) {
    return {
      errors: [
        {
          path: 'skillProficiencyId',
          code: 'DUPLICATE',
          message: `A skill proficiency with id "${skillProficiencyId}" already exists in this campaign`,
        },
      ],
    };
  }
...
export async function update(
  ...
  | { skillProficiency: CampaignSkillProficiencyDoc }
...
export async function remove(
  ...
  const result = await CampaignSkillProficiency.deleteOne({
    campaignId,
    skillProficiencyId,
  });
