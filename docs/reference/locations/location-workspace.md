...
This document describes the **full-width create/edit shell** for locations in a campaign: layout components under `components/workspace/`, **building + floor** editing, and **shared canvas** hooks used by the map editor. For the broader locations domain (mental model, shared `grid/` and `locations/` packages, validation, server layout), see [domain.md](./domain.md). **Locations reference index:** [README.md](./README.md). **Placed objects (pipeline, single narrative):** [placed-objects-flow.md](./placed-objects-flow.md).
...
**Authored base-map layer order (square + hex):** Named z-indices live in `components/workspace/mapAuthoringLayerZ.ts` (`MAP_AUTHORING_LAYER_Z`). **Square:** terrain swatches (detached CSS grid) → **paths + edges** (`SquareMapAuthoringSvgOverlay`) → interactive grid chrome (`GridEditor` with `omitTerrainFill`, plus per-cell region tint / links) → **global** placed-object glyphs (`LocationGridAuthoringSquareMapPlacedObjectsOverlayLayer`). **Hex:** cell grid (terrain inside cells) → path splines above cells → global glyphs → region hull outlines. Region tint on square stays in `LocationMapCellAuthoringOverlay`; optional future work is splitting hex terrain into the same bottom slot as square for identical numeric stacking. **Edge** doors/windows on square are **vector** segments on boundaries. Object display rules align with `deriveLocationMapAuthoredObjectRenderItems` / `LocationMapAuthoredObjectRenderItem` in `shared/domain`; resolution to URLs and layout is `resolvePlacedObjectCellVisual` → `PlacedObjectCellVisualDisplay` in `domain/presentation/map/`.
...
App-wide MUI theme (`palette`, etc.) still applies; map-specific tuning should go through these modules rather than ad hoc values in components. See [color-theming.md](../color-theming.md) and [domain.md](./domain.md) (grid renderers + styling parity).
...
**Whitespace / normalization:** See **Normalization policy (dirty vs save)** above and `LOCATION_WORKSPACE_NORMALIZATION` in `locationWorkspaceNormalizationPolicy.ts`. Plan archive: [location_workspace_normalization_policy.plan.md](../../../.cursor/plans/location-workspace/location_workspace_normalization_policy.plan.md).
...
**Placed objects — end-to-end flow (registry, manifest, URLs, persistence, layout, rendering):** **[placed-objects-flow.md](./placed-objects-flow.md)** — canonical narrative so this workspace doc does not duplicate pipeline detail. **Asset commands / CI / artist checklist:** [`assets/system/locations/objects/README.md`](../../../assets/system/locations/objects/README.md). **Sprite migration phases:** [.cursor/plans/sprite_placed_objects_migration_a4f3eeec.plan.md](../../../.cursor/plans/sprite_placed_objects_migration_a4f3eeec.plan.md).
