...
**Edit route composition:** `LocationEditRoute` loads the entry, then calls **`useLocationEditWorkspaceModel`** (`routes/locationEdit/useLocationEditWorkspaceModel.ts`) for form state, grid draft, map editor, palettes, canvas zoom/pan, and handlers. **Hydration** (`useLocationMapHydration`) wraps `hydrateDefaultLocationMapState` for non-building vs building-floor maps. **Save / patch / add floor** (`useLocationEditSaveActions`) centralizes **homebrew** submit (`handleHomebrewSubmit`), `useSystemPatchActions`, and floor creation. Map and Selection rail **panel components** (`LocationEditWorkspaceMapAuthoringRailPanel`, `LocationEditWorkspaceSelectionRailPanel`) live in **`routes/locationEdit/locationEditWorkspaceRailPanels.tsx`**; the route wires props and passes `mapAuthoringPanel`, `selectionPanel`, and `mapCanvasColumn` into **`LocationEditHomebrewWorkspace`** or `LocationEditSystemPatchWorkspace`.
...
1. **Region metadata (Selection → region):** [`LocationMapRegionMetadataForm`](src/features/content/locations/components/workspace/rightRail/selection/LocationMapRegionMetadataForm.tsx) leads with [`SelectionRailIdentityBlock`](src/features/content/locations/components/workspace/rightRail/selection/PlacedObjectRailTemplate.tsx) (**Map** / region **name** or “Region” / **Overlay region**), then uses RHF for field UX; **`onPatchRegion(regionId, patch)`** writes through [`regionMetadataDraftAdapter.ts`](src/features/content/locations/components/workspace/rightRail/adapters/regionMetadataDraftAdapter.ts) into **`gridDraft.regionEntries`**. **Description** uses [`useDebouncedPersistableField`](src/ui/hooks/useDebouncedPersistableField.ts) for typing ergonomics, with **flush** on region change (`useLayoutEffect`), rail tab unmount, and via **`flushDebouncedPersistableFieldsRef`** before header Save (wrapped in **`flushSync`** in [`LocationEditRoute.tsx`](src/features/content/locations/routes/LocationEditRoute.tsx) so `gridDraftRef` matches before submit). No separate “apply” step for persistable data.
...
3. **Placed objects (Selection → object / edge):** [`PlacedObjectRailTemplate`](src/features/content/locations/components/workspace/rightRail/selection/PlacedObjectRailTemplate.tsx) is the shared **object-first** rail layout for placed cell objects and boundary edges (door/window/wall). Order: **category** (palette category label) → **object title** (registry or legacy kind) → **placement** (cell coordinates or edge id) → **metadata** (kind-specific, e.g. stairs; door/window **edge** segments use **presentation** rows from the family default variant via [`getDefaultVariantPresentationForKind`](src/features/content/locations/domain/model/placedObjects/locationPlacedObject.selectors.ts) and [`presentationRowsFromPresentation`](src/features/content/locations/components/workspace/rightRail/selection/placedObjectRail.helpers.ts); cell objects may resolve kind via [`resolvePlacedObjectKindForCellObject`](src/features/content/locations/domain/model/placedObjects/locationPlacedObject.selectors.ts) where needed) → **Label** (freeform when the cell is not using **linked display identity**) or **linked campaign location name** when the placed-object family has **`linkedScale`** in the registry and the cell’s linked location matches that scale → **Remove from map**. Pure helpers: [`placedObjectRail.helpers.ts`](src/features/content/locations/components/workspace/rightRail/selection/placedObjectRail.helpers.ts). **Empty-cell** selection uses only [`SelectionRailIdentityBlock`](src/features/content/locations/components/workspace/rightRail/selection/PlacedObjectRailTemplate.tsx) plus host/ancestry context in [`LocationCellAuthoringPanel`](src/features/content/locations/components/workspace/rightRail/panels/LocationCellAuthoringPanel.tsx) — **no** linked-location picker or add/remove cell objects there (those belong on object selection and other tools). **Edge** door/window inspectors show a disabled **Label** and a short persistence note: today the map wire stores **`edgeId` + `kind`** only; extended edge-authored fields are planned (see [.cursor/plans/location-workspace/location_workspace_object_authoring_phase4_config_editing.plan.md](../../.cursor/plans/location-workspace/location_workspace_object_authoring_phase4_config_editing.plan.md)).
4. **Paths (Selection → path):** [`LocationMapPathInspector`](src/features/content/locations/components/workspace/rightRail/selection/LocationMapSelectionInspectors.tsx) uses the same **PlacedObjectRailTemplate** backbone (no raw path id in the primary block).
...
| `place` | **Discrete** placement only: **linked content** (child locations) and **map objects** (props), per `LOCATION_SCALE_MAP_CONTENT_POLICY`. Palette: `getPlacePaletteItemsForScale` → toolbar **`LocationMapEditorPlaceTray`**; selection **`activePlace`** (`linked-content` \| `map-object`, **`kind`** = family id, required **`variantId`**). Cell click: **`resolvePlacementCellClick`** (`placementRegistryResolver.ts`). Map rail: hint only (no duplicate palette). |
| `draw` | **Line/boundary** authoring: **paths** and **edges** (per `LOCATION_SCALE_MAP_CONTENT_POLICY`). Palette: `getGroupedDrawPaletteForScale` → toolbar `LocationMapEditorDrawTray` + Map rail `LocationMapEditorDrawPanel`; selection `activeDraw` (`path` \| `edge`). **Edge** rows use **`LOCATION_EDGE_FEATURE_KIND_META`** for **label/description only** (coarse `kind` copy — not variants or instance state). Path segments use `pathAnchorCellId`; edges use boundary-paint in `LocationGridAuthoringSection`. Resolver: `resolveDrawSelectionToAction`. |
...
| `LocationMapEditorPlacePanel` | Legacy grid layout for place items (same data shape); not used in the Map rail in Phase 1 — prefer **`LocationMapEditorPlaceTray`** for toolbar. |
