...
3. **Placed objects (Selection → object / edge):** [`PlacedObjectRailTemplate`](src/features/content/locations/components/workspace/rightRail/selection/PlacedObjectRailTemplate.tsx) is the shared **object-first** rail layout for placed cell objects and boundary edges (door/window/wall). Order: **category** (palette category label) → **object title** (registry or legacy kind) → **placement** (cell coordinates or edge id) → **metadata** (kind-specific, e.g. stairs) → **Label** (freeform when the cell is not using **linked display identity**) or **linked campaign location name** when the placed-object family has **`linkedScale`** in the registry and the cell’s linked location matches that scale → **Remove from map**. Pure helpers: [`placedObjectRail.helpers.ts`](src/features/content/locations/components/workspace/rightRail/selection/placedObjectRail.helpers.ts). **Do not** use the template for empty-cell selection (`LocationCellAuthoringPanel` remains the cell inspector).
