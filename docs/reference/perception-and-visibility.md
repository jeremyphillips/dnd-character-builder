...
**Grid occupant tokens (active viewer):** `selectGridViewModel` (when `GridPerceptionInput` is passed) sets **`viewerOccupantPresentationKind`** and **`viewerPerceivesOccupantToken`** per occupied cell via **`deriveViewerCombatantPresentationKind`** / **`shouldRenderOccupantTokenForEncounterViewer`** (`encounter/space/grid-occupant-render-visibility.ts`). Presentation kinds are **`visible`** | **`out-of-sight`** | **`hidden`** (see precedence in that module). Under **strict POV**, **`viewerPerceivesOccupantToken`** is true only for **`visible`** (same as before: non-visible kinds suppress the normal token). Cell-level **`occupantTokenVisibility`** (blind veil, darkness, etc.) is still applied in **`EncounterGrid`** after the pair presentation. **DM / `viewerRole: 'dm'`** forces **`visible`**. **Guessed-position** is not implemented; the type can be extended later without changing rule engines.
