...
3. **Workspace structure (incremental)** — [location_workspace_ownership_reorg.plan.md](location_workspace_ownership_reorg.plan.md): ownership subtrees under `components/workspace/` (`header/`, `setup/`, `canvas/`, `leftTools/`, `rightRail/`), why `LocationGridAuthoringSection` stays workspace-level (not a pure canvas child), subtree `__tests__/`, and phased moves without mega-renames.
