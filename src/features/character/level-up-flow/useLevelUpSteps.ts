...
import { getClassProgression, getSubclassUnlockLevel } from '@/features/character/domain/progession'
...
export function useLevelUpSteps(state: LevelUpState): LevelUpStepConfig[] {
  ...
  return useMemo(() => {
    ...
    if (prog?.spellProgression) {
      const oldLimits = getSpellLimits(prog, oldClassLevel)
      const newLimits = getSpellLimits(prog, newClassLevel)
